...
void event_loop(int sockfd, struct io_uring *ring)
{
    ...
    for (;;)
    {
        ...
        {
        case FLAG_ACCEPT:
    	    if (server_logging_enabled) zlog_info(server_log, "server accept incoming client with socket %d",cqe->res);
        ...
        case TEST_RESPONSE:
            ...
            { // non-empty request?  set fd test not zero read
    	        if (server_logging_enabled) zlog_info(server_log, "testing client responce, readed %d bytes",cqe->res);
                ...
                case (IPC_MESSAGE__STATUS__NEED_MORE):
    	            if (server_logging_enabled) zlog_info(server_log, "client needs more messages");
                    handle_response_NEED_MORE_MSG(ring, cqe->user_data);
                    break;
                case (IPC_MESSAGE__STATUS__ACKN_OK):
    	            if (server_logging_enabled) zlog_info(server_log, "client acknowledged the message");
