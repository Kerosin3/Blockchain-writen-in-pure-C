...
void request_ASK_NEED_MSG(struct io_uring *ring, int client_fd)
{
    if (server_logging_enabled)
        zlog_info(server_log, "asking whether client need a msg");
...
void FINISH_SENDING(struct io_uring *ring, int client_fd)
{
    ...
    if (server_logging_enabled)
        zlog_info(server_log, "calcin merkle root");
...
{
    if (server_logging_enabled)
        zlog_info(server_log, "SENDED TO CURRENT CLIENT %lu", beffer_sended_N[client_fd]);
    if (beffer_sended_N[client_fd] >= N_MESSAGES)
    { // block filled
        request_SEND_STATUS(ring, client_fd, IPC_MESSAGE__STATUS__ALL_BLOCK_MSG_SENDED);
        if (server_logging_enabled)
            zlog_info(server_log, "server sent all messages!!!!!!!!!!!!!!");
        return;
    }
    struct io_uring_sqe *sqe = io_uring_get_sqe(ring);    // add to ring
    ...
    if (server_logging_enabled)
        zlog_info(server_log, "send signed message size = %zu serial: %lu ", buffer_lengths[client_fd],
