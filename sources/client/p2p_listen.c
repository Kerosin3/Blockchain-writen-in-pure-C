...
void setup_p2p_listening(char *IP_ADD_LISTEN)
{
    ...
    {
        if (p2p_logging_enabled)
            zlog_info(p2p_log, "getaddrinfo error:%s\n", gai_strerror(status));
        thrd_exit(1);
    }
    ...
    if (p2p_logging_enabled)
        zlog_info(p2p_log, "p2p client tryint to connect...");

    if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &(int){1}, sizeof(int)) < 0)
        if (p2p_logging_enabled)
            zlog_info(p2p_log, "error setting socket reuse");
    int rezopt = fcntl(s, F_SETFL, fcntl(s, F_GETFL, 0) | O_NONBLOCK);
    if (rezopt == -1)
    {
        if (p2p_logging_enabled)
            zlog_info(p2p_log, "error error calling fcntl");
    }
    if (p2p_logging_enabled)
        zlog_info(p2p_log, "have setted socket nonblock...");
    if (p2p_logging_enabled)
        zlog_info(p2p_log, "locking mutex connection accepted...");
    int done = 1;
    while (1)
    {
    ...
    if (p2p_logging_enabled)
        zlog_info(p2p_log, "setting socket nonblock...");
    printf("GOT CONNECTION!\n");
    if (p2p_logging_enabled)
        zlog_info(p2p_log, "got connection to p2p server");
    ...
    { // submit
        if (p2p_logging_enabled)
            zlog_info(p2p_log, "error submitting");
    }
    for (;;)
    {
        if (kill_thread_p2p)
        ...
        case PROCESS_RESPONSE: // RECEIVING A MESSAGE
            ...
            case P2P__IPC_MESSAGE__STATUS__PING: // server sent PING -> answer pong
                ...
                if (p2p_logging_enabled)
                    zlog_info(p2p_log, "p2p client pong sended");
                break;
            case P2P__IPC_MESSAGE__STATUS__OK:
                if (p2p_logging_enabled)
                    zlog_info(p2p_log, "p2p server answered OK");
                ...
                if (p2p_logging_enabled)
                    zlog_info(p2p_log, "p2p client asks for a block");
                break;
        ...
        case READ_RESPONSE:
            ...
            if (p2p_logging_enabled)
                zlog_info(p2p_log, "reading p2p server response");
            break;
                        ...
                        cl_p2p_state STATE_MACHINE)
{
    ...
    if (io_uring_submit(ring) < 0)
        printf("error submitting\n");
    if (p2p_logging_enabled)
        zlog_info(p2p_log, "sent PONG msg");
}
