...
void event_loop_p2p(event_p2p_params_t *elparams)
{
    ...
    if (p2p_logging_enabled)
        zlog_info(p2p_log, "logging p2p application");
    ...
    for (;;)
    {
        ...
        if (mtx_trylock(&block_created_mtx) == thrd_success)
        {   // test if thread created block!
        ...
        case FLAG_ACCEPT:
            ...
            if (p2p_logging_enabled)
                zlog_info(p2p_log, "accepted a connection");
        ...
        case FLAG_TEST_RESPONSE:
            if ((cqe->res) > 0)
            {
                ...
                case P2P__IPC_MESSAGE__STATUS__PONG: // received PONG --> ask if block is ready
                    ...
                    if (p2p_logging_enabled)
                        zlog_info(p2p_log, "p2p got pong response");
                    P2Pser_send_STATUS(ring, request_data_client_fd(cqe->user_data), P2P__IPC_MESSAGE__STATUS__OK,
                                       FLAG_READ_RESPONSE);
                    if (p2p_logging_enabled)
                        zlog_info(p2p_log, "sent ok");
                    break;
                case P2P__IPC_MESSAGE__STATUS__ASK_IF_BLOCK_READY: // IM BEING ASKED
                    printf("server being asked if I have a block ready\n");
                    if (p2p_logging_enabled)
                        zlog_info(p2p_log, "p2p server being asked if has block ready");
                    if (flag_block_ready)
                    {
                        printf("block ready!\n");
                        P2Pser_send_block(ring, request_data_client_fd(cqe->user_data));
                        if (p2p_logging_enabled)
                            zlog_info(p2p_log, "block sended!");
                    }
                    else
                    {
                        if (p2p_logging_enabled)
                            zlog_info(p2p_log, "block NOT ready!");
        ...
        case FLAG_READ_RESPONSE:
            P2P_read_status_response(ring, request_data_client_fd(cqe->user_data)); // recv
            if (p2p_logging_enabled)
                zlog_info(p2p_log, "reading response");
            break;
        case FLAG_BLOCK_SENDED:
            printf("block has been sent!\n");
            P2Pser_send_STATUS(ring, request_data_client_fd(cqe->user_data), P2P__IPC_MESSAGE__STATUS__FINISH,
                               FLAG_WAIT_PONG);
            if (p2p_logging_enabled)
                zlog_info(p2p_log, "black has been sent, sent finish, wait pong ");
    ...
    if (p2p_logging_enabled)
        zlog_info(p2p_log, "exiting main p2p process!");
