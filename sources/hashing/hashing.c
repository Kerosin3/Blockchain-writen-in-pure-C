...
unsigned char *calc_hash(signed_message_t s_message)
{
    ...
    crypto_generichash(out_hash, crypto_generichash_BYTES, s_message.message, s_message.length, NULL, 0);
    return out_hash;
}
...
{
    unsigned char *out_hash = calloc(crypto_generichash_BYTES, sizeof(unsigned char));
    crypto_generichash(out_hash, crypto_generichash_BYTES, hash0, crypto_generichash_BYTES, NULL, 0);
    return out_hash;
}
